...
Most category 1 assessments are based on equations ([-@eq-grow])--([-@eq-plus]), with additional features [e.g., process error in @sam]. The same equations are commonly the basis to give catch advice, calculate reference points (e.g. MSY) and perform management strategy evaluations (MSE).
...
the model. @fig-ageingmizer demonstrates the process, showing how, for a given species, age is calculated.

```{r echo=FALSE, out.width="500px",out.height="500px", fig.align='center', fig.cap="Demonstration of how we calculate weights for immortal individuals across time. Each line represents the weight of one II, each introduced annually."}
#| label: fig-ageingmizer
knitr::include_graphics("pictures/aging_mizer_toy.png")
```

We solve equation ([-@eq-IIpde]) in mizer for time `dt` numerically given weight classes `w`, with differences `dw`. If at the current time the II is size `cur_waa` then it will be in size class
...
@fig-aging-weights shows the emergent weight at age for whiting in mizer.

```{r echo=FALSE, out.width="500px",out.height="500px", fig.align='center', fig.cap="Emergent growth curve for whiting."}
#| label: fig-aging-weights
...
```{r echo=FALSE, out.width="80%", fig.align='center', fig.cap="The black line shows the weight at age inputs to SMS' key-run [@ICES2024] (and the stock assessent) for whiting and the red line shows the mizer estimate of weight at age for whiting using `NS_params`"}
#| label: fig-historic-weights
knitr::include_graphics("pictures/whiting_obs_miz.png")
```

In @fig-historic-weights we show the size at age of whiting and compare it to weight at age inputs to SMS' key-run [@ICES2024]. Using the default mizer model `NS_params` we show that the weight at age of whiting changes over the simulation, with a similar pattern to the inputs to SMS, however, the scale and range are different.
...
```{r echo=FALSE, out.width="500px", out.height="700px", fig.align='center', fig.cap="The top plot shows the evolution of individual whiting cohorts through time. The lower plots shows the fishing mortality over time for whiting from @spence_prec"}
...
By calculating the numbers at age, as done in the section '[Numbers at age](#sec:numatage)', we can calculate how many individuals die in a cohort in a year. Consider that at time $t-1$ and $t$ we have calculated numbers age $a-1$ and $a$ respectively, then we can rearrange equation ([-@eq-grow]) to find
$$
...
We calculate equations ([-@eq-getZ]) and ([-@eq-getF]) for the `i`th year, `j`th species and `k`th age class to get $Z_{i,k}$ and $F_{i,k}$
...
and equation ([-@eq-getM]) to get $M_{i,k}$

```{r,eval=FALSE}  
m <- z - F_m
```

@fig-Z-plot is the emergent $Z$, $F$ and $M$ over time using the historical fishing mortality.

```{r echo=FALSE, out.width="80%", fig.align='center', fig.cap="Plots depict the $Z$, $F$ and $M$ rates for whiting of different ages over time"}
#| label: fig-Z-plot
...
```{r echo=FALSE, out.width="500px",out.height="500px", fig.align='center', fig.cap="Comparison of II size prediction to mizer's numerical solution. The grid cell value is the proportion of individuals in the numerical solution that are smaller than the II. Each simulation has a different `dt` and `no_w`."}
#| label: fig-quantiles
knitr::include_graphics("pictures/quantiles.png")
```

```{r echo=FALSE, out.width="600px",out.height="600px",  fig.align='center', fig.cap="The numerical solution of the growth with different `dt` and `no_w`. Each plot contains 3 lines, indicating a different value of `dt` - black is 0.1, blue is 0.01 and red is 0.001. Each row of plots indicates that the example's `no_w`, increasing from top to bottom. The left columns shows age 1 and right age 4."}
#| label: fig-sensitive
knitr::include_graphics("pictures/SENSITIVEGRID.png")
```

We compared the proportion of individuals that are smaller than the II and found that as `dt` got smaller and `no_w` got bigger the proportion approached 50%, @fig-quantiles. Further, as `dt` got smaller and `no_w`got bigger the range of sizes at age decreased @fig-sensitive.

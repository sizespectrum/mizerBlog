...
#> [1] 0.07473097
stab$eigenvalues[1]
#> [1] 0.07473097+1.249195i
stab$dominant_period
#> [1] 5.029789
```

The leading eigenvalue is complex with a positive real part. A real positive eigenvalue would mean monotone growth away from the steady state; a complex pair means the perturbation grows *while oscillating* — here by a factor of $e$ every thirteen years or so — and the imaginary part sets the period, about 5.0 years. Fish and resource are perturbed together, so this rests on nothing but the model.
...
Below one. mizer's default `euler` step at `dt = 0.1` does not merely mis-state this instability, it very nearly cancels it: the model grows at $+0.075$ per year, the scheme at $-0.002$ per year. Project the model that way for a century and it sits flat, looking for all the world like a stable steady state. mizer solves the transport implicitly, and implicit schemes damp oscillations artificially; here the artificial damping happens to be almost exactly the size of the thing being measured. Nor does it fail in a tidy direction as the step coarsens — at $\Delta t = 1$ the same map is unstable again, with a spectral radius of 1.14, but oscillating at a 3.4-year period belonging to no mode of the model. That is why the run below uses `method = "tr_bdf2"`, and why `getStability()` no longer goes anywhere near the one-step map.

(Both functions perturb the resource alongside the fish. An earlier design let you substitute it at its quasi-static equilibrium instead, which is harmless for the model — it moves the leading eigenvalue by 3% — but not for a one-step map, whose resource responds with a whole step's lag rather than instantly; at $\Delta t = 1$ that shortcut flips the verdict. The option is gone.)

The two numbers cannot be converted into one another by any exact algebraic relation, which is why `getStability()` differentiates the rates of change directly instead of going through the one-step map. Were mizer's step fully implicit, $\mu = 1/(1 - \Delta t\,\lambda)$ would let you invert it — but the rates are evaluated at the state at the *start* of the step and only the transport solve that follows is implicit, so that inversion is correct only to first order in $\Delta t$, and at $\Delta t = 1$ it is wrong by more than an order of magnitude ([#556](https://github.com/sizespectrum/mizer/issues/556)).
